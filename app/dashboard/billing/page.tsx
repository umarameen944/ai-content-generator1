...
function billing() {
  ...
  const CreateSubscription=()=>{
    setLoading(true)
    axio.post('/api/creat-subscription',{})
    .then(resp=>{
  ...
  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:items-center md:gap-8">
    ...
    <div className="rounded-2xl bg-white border border-gray-200 p-6 shadow-sm sm:px-8 lg:p-12">
      <div className="text-center">
        ...
        <p className="mt-2 sm:mt-4">
          <strong className="text-3xl font-bold text-gray-900 sm:text-4xl"> 100 INR </strong>
...
{/* 
  ...
  disabled={loading}
  onClick={() => CreateSubscription()}
  className='w-full rounded-full mt-5 p-6 border border-indigo-600'
  style={{
    backgroundColor: userSubscription?'lightgreen':'lightblue',
  }}
  variant='outline'
>
  {loading && <Loader2Icon className='animate-spin' />}
